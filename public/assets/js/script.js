(function() {
  ...
  image.src = config.custom_image || 'assets/images/banner.png';
  ...
  function selectDisplayMode(mode) {
    if (mode === 'one') {
      ...
      console.log('Switched to One Image Mode');
    } else if (mode === 'many') {
      manyImagesBtn?.classList.add('selected');
      oneImageBtn?.classList.remove('selected');
      canvasContainer.style.display = 'none';
      galleryContainer.style.display = 'flex';
      loadGalleryImages(adminConfigs);
      console.log('Switched to Many Images Mode');
    }
  }

  // Function to load and display images in the gallery
  function loadGalleryImages(configs) {
    if (!galleryContainer) return;
    galleryContainer.innerHTML = ''; // Clear previous images

    const templates = Object.values(configs);
    ...
    templates.forEach(template => {
      if (template.image_data) {
        ...
        img.alt = template.template_name || 'Template Image';
  ...
  downloadBtn?.addEventListener('click', () => {
    ...
    const dir   = directionSelect.value.trim().toUpperCase();
    const name  = `${promo}_${geo}_${cur}_${dir}.png`;
